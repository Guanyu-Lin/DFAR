...
__all__ = ["BaseModel_contrastive"]


class BaseModel_Contrastive:
    ...
    def __init__(self, hparams, iterator_creator, graph=None, seed=None):
        ...
        with self.graph.as_default():
            ...
            # self.rnn_outputs, self.rnn_outputs_user, self.logit_u, self.logit_i, self.logit_pos = self._build_graph()
            self.logit_pos, self.logit_neg, self.loss_cosine, self.loss_cosine_long, self.score_pos, self.score_neg, self.att_score_f = self._build_graph()
            # import pdb
            # pdb.set_trace()
            self.pred_bpr = tf.cast(tf.constant(2), tf.float32)*(tf.cast(self.iterator.labels, tf.float32) - tf.constant(0.5))*(self.logit_pos - self.logit_neg)

            
            self.predA = self._get_pred(self.logit_pos, self.hparams.method)
            # self.pred_neg = self._get_pred(self.logit_neg, self.hparams.method)
    ...
    def _get_lossA(self):
        ...
        Returns:
            obj: Loss value
        """
        components = self.hparams.components.split(',')
        print(components)
        ...
        self.lossA = tf.add(self.data_lossA, self.regular_loss) 
        self.loss_cosine = tf.reduce_mean(self.loss_cosine)
        self.loss_cosine_long = tf.reduce_mean(self.loss_cosine_long)
        self.contrastive_loss = self._contrastive_loss()
        self.bpr_loss = self._bpr_loss()

        self.lossA = tf.add(self.lossA, self.loss_cosine)
        if not self.hparams.loss == "bpr_loss":
            self.lossA = tf.add(self.lossA, float(components[2]) * self.contrastive_loss)
        # self.lossA = tf.add(self.lossA, float(components[0]) * self.bpr_loss)
        self.lossA = tf.add(self.lossA, float(components[1]) * self.loss_cosine_long)

        return self.lossA
    def _bpr_loss(self):
        regular_para = 0

        return regular_para * tf.reduce_mean(-tf.log(tf.nn.sigmoid(tf.cast(tf.constant(2), tf.float32)*(tf.cast(self.iterator.labels, tf.float32) - tf.constant(0.5))*(self.score_pos - self.score_neg))))
    ...
    def _add_summariesA(self):
        # tf.summary.scalar("data_loss", self.data_lossA)
        # tf.summary.scalar("regular_loss", self.regular_loss)
        # tf.summary.scalar("loss", self.lossA)
        print("shs")
        merged = tf.summary.merge_all(key='summaries')
        return merged

    def _contrastive_loss(self):
        regular_para = self.hparams.loss_l2

        l2_loss = tf.zeros([1], dtype=tf.float32)
        if not self.hparams.loss == "bpr_loss":
            print(self.hparams.loss)
            print("restrict prediction")
            # l2_loss = tf.add(
            #     l2_loss, tf.multiply(regular_para, tf.nn.l2_loss(self.logit_pos - self.logit_neg))
            # )
            l2_loss = tf.add(
                l2_loss, tf.multiply(regular_para, tf.reduce_mean(-tf.log(tf.nn.sigmoid(self.pred_bpr))))
            )
            # data_loss =  

        return tf.reduce_sum(l2_loss)
        ...
        if self.hparams.loss == "cross_entropy_loss":
            data_loss = tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(
                    logits=tf.reshape(self.logit_pos, [-1]),
            ...
            print(self.hparams.loss)
            data_loss =  tf.reduce_mean(-tf.log(tf.nn.sigmoid(self.pred_bpr)))
            
        elif self.hparams.loss == "softmax":
            group = self.train_num_ngs + 1
            logits = tf.reshape(self.logit_pos, (-1, group))
    ...
    def _build_train_optA(self):
        ...
        gradients, variables = zip(*train_step.compute_gradients(self.lossA))
        # print("i am gradients A",gradients) 
