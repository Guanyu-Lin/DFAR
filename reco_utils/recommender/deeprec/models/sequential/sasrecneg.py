...
class SASRecNegModel(SLI_RECModel):
...
#
    def _build_seq_graph(self):
        ...
        return model_output, attn_score
    ...
    def multihead_attention(self, queries, 
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            Q_ = tf.tile(tf.expand_dims(Q_, 2), [1, 1, K.get_shape().as_list()[1], 1])
            K_ = tf.tile(tf.expand_dims(K_, 1), [1, Q.get_shape().as_list()[1], 1, 1])
            outputs = tf.squeeze(tf.layers.dense(Q_ - K_, 1, activation=None)) # (h*N, T_q, T_k, 1)
            # Multiplication
