...
from reco_utils.recommender.deeprec.models.sequential import util
...
def create_projection_matrix(m, d, seed=0, scaling=0, struct_mode=False):
  ...
  block_list = []
  current_seed = seed
  #  current_seed = tf.to_int32(seed)
  for _ in range(nb_full_blocks):
    if struct_mode:
      q = create_products_of_givens_rotations(d, seed)
    else:
      #  要求必须传入int，但暂时无法处理，遂取消设置seed
...
class Attention(tf.keras.layers.Layer):
  ...
  def __init__(self,
               ...
               #  nb_random_features=350):
               #  nb_random_features=5):
...
class SelfAttention(Attention):
  ...
  def call(self,
    ...
    #  return super(SelfAttention, self).call(query_input, source_input, bias,
