...
class SASRecFeedDualModelMASK(SLI_RECModel_Dual):
...
#
    def _build_seq_graph(self):
        ...
        with tf.name_scope('sasrec'):
            ...
            # attention_output, alphas = self._attention_fcn(self.target_item_embedding + self.pos_label_embed, self.seq, 'Att', False, return_alpha=True)
...
#              #  self.recent_embedding_sum = tf.reduce_sum(self.seq*tf.expand_dims(self.real_recent_mask, -1), 1)
        ...
        return model_output_pos, model_output_neg, self.cosine_distance(self.hist_embedding_mean_pos, self.hist_embedding_mean_neg), self.cosine_distance(att_fea_pos, att_fea_neg), tf.reduce_mean(score_pos), tf.reduce_mean(score_neg), attn_score_f
    ...
    def multihead_attention(self, queries, 
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            Q_pos = self.pos_mask * tf.layers.dense(queries, num_units, activation=None) # (N, T_q, C)
            K_pos = self.pos_mask * tf.layers.dense(keys, num_units, activation=None) # (N, T_k, C)
            V_pos = self.pos_mask * tf.layers.dense(keys,  num_units, activation=None) # (N, T_k, C)
            Q_neg = self.neg_mask * tf.layers.dense(queries,   num_units, activation=None) # (N, T_q, C)
            K_neg = self.neg_mask * tf.layers.dense(keys,  num_units, activation=None) # (N, T_k, C)
            V_neg = self.neg_mask * tf.layers.dense(keys, num_units, activation=None) # (N, T_k, C)
            Q = Q_pos + Q_neg
            K = K_pos + K_neg
            V = V_pos + V_neg
            # Q = Q_pos 
            ...
            outputs = tf.layers.dense(outputs, initial_units, activation=None)
            # Residual connection
            outputs += queries
