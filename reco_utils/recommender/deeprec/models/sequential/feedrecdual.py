...
class SASRecFeedDualModel(SLI_RECModel_Dual):
...
#              #  self.recent_embedding_sum = tf.reduce_sum(self.seq*tf.expand_dims(self.real_recent_mask, -1), 1)
        ...
        return model_output_pos, model_output_neg, self.cosine_distance(self.hist_embedding_mean_pos, self.hist_embedding_mean_neg), self.cosine_distance(att_fea_pos, att_fea_neg), tf.reduce_mean(score_pos), tf.reduce_mean(score_neg)
