...
__all__ = ["SASRecFeedModel"]


class SASRecFeedModel(SequentialBaseModel_Contrastive_noLong):
...
#              #  self.recent_embedding_sum = tf.reduce_sum(self.seq*tf.expand_dims(self.real_recent_mask, -1), 1)
        ...
        model_output = tf.concat([self.target_item_embedding, self.hist_embedding_mean, self.hist_embedding_mean_pos, self.hist_embedding_mean_neg], -1)
        #  import pdb; pdb.set_trace()
        ...
        return model_output, self.cosine_distance(self.hist_embedding_mean_pos, self.hist_embedding_mean_neg)
