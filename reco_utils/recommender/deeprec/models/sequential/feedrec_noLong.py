...
__all__ = ["SASRecFeedDualModel"]


class SASRecFeedDualNoLongModel(SLI_RECModel_Dual):
...
#
    def _build_seq_graph(self):
        ...
        with tf.name_scope('sasrec'):
            ...
            self.neg_mask = tf.cast(1 - self.iterator.user_history, tf.float32)
            ...
            self.seq_pos = self.seq * tf.expand_dims(self.pos_mask, -1)
            self.seq_neg = self.seq * tf.expand_dims(self.neg_mask, -1)
            ...
            for i in range(self.num_blocks):
                ...
                #                                    scope="self_attention")]
                    ...
                    self.seq_pos = self.feedforward(self.normalize(self.seq_pos), num_units=[self.hidden_units, self.hidden_units],
                                           dropout_rate=self.dropout_rate, is_training=self.is_training)
                    self.seq_pos *= tf.expand_dims(self.pos_mask, -1)
            ...
            for i in range(self.num_blocks):
                ...
                #                                    scope="self_attention")]
                    ...
                    self.seq_neg = self.feedforward(self.normalize(self.seq_neg), num_units=[self.hidden_units, self.hidden_units],
                                           dropout_rate=self.dropout_rate, is_training=self.is_training)
                    self.seq_neg *= tf.expand_dims(self.neg_mask, -1)
...
#              #  self.recent_embedding_sum = tf.reduce_sum(self.seq*tf.expand_dims(self.real_recent_mask, -1), 1)
        ...
        model_output_pos = tf.concat([self.target_item_embedding, self.hist_embedding_mean, self.hist_embedding_mean_pos], -1)
        model_output_neg = tf.concat([self.target_item_embedding, self.hist_embedding_mean, self.hist_embedding_mean_neg], -1)
        ...
        return model_output_pos, model_output_neg, self.cosine_distance(self.hist_embedding_mean_pos, self.hist_embedding_mean_neg), self.cosine_distance(att_fea_pos, att_fea_neg), tf.reduce_mean(score_pos), tf.reduce_mean(score_neg), attn_score
