...
def create_hparams(flags):
    ...
    return tf.contrib.training.HParams(
        ...
        loss_l2=flags["loss_l2"] if "loss_l2" in flags else 0.0000,
        ...
        num_heads=flags["num_heads"] if "num_heads" in flags else None,
...
def mrr_score(y_true, y_score):
    ...
    #     import pdb
    #     pdb.set_trace
    # print(np.sum(rr_score) / np.sum(y_true))
...
def cal_weighted_metric(users, preds, labels, metrics):
    ...
    # import pdb
    # pdb.set_trace()
    # gb_list = df.groupby("users")[df.groupby("users").count() > 2]
    # index = df.groupby("users").count() > 2
    # df = pd.DataFrame({'users': users, 'preds': preds, 'labels': labels})
