...
class SequentialIterator(BaseIterator):
    def __init__(self, hparams, graph, col_spliter="\t"):
        ...
        with self.graph.as_default():
            ...
            self.user_history = tf.placeholder(
                tf.int32, [None, self.max_seq_length], name="user_history"
            )
            ...
            self.mask_u = tf.placeholder(
                tf.int32, [None, self.max_seq_length], name="mask_u"
            )
    ...
    def parse_file(self, input_file):
        ...
        for line in lines:
            if not line:
                continue
            # print("line", linie)
            pased_line = self.parser_one_line(line)
            # if pased_line:
            res.append(pased_line)
        return res
    ...
    def parser_one_line(self, line):
        ...
        user_history_words = words[5].strip().split(",")
        # import pdb; pdb.set_trace()
        item_history_words = words[6].strip().split(",")
        cate_history_words = words[7].strip().split(",")
        user_history_sequence, item_history_sequence, cate_history_sequence = self.get_item_cate_history_sequence(user_history_words, item_history_words, cate_history_words, user_id)

        time_history_words = words[8].strip().split(",")
    ...
    def get_item_cate_history_sequence(self, user_history_words, item_history_words, cate_history_words, user_id):

        user_history_sequence = self.get_user_history_sequence(user_history_words)
        ...
        return user_history_sequence, item_history_sequence, cate_history_sequence
    def get_user_history_sequence(self, user_history_words):

        user_history_sequence = []
        for user in user_history_words:
            # import pdb; pdb.set_trace()
            user_history_sequence.append(
                user
            )
        return user_history_sequence
    ...
    def load_data_from_file(self, infile, batch_num_ngs=0, min_seq_length=1):
        ...
        for line in lines:
            ...
            if len(item_history_sequence) < min_seq_length or len(user_history_sequence) < min_seq_length:
                continue
            # print(user_history_sequence)
            # print(len(user_history_sequence))
            label_list.append(label)
            user_list.append(user_id)
            item_list.append(item_id)
            item_cate_list.append(item_cate)
            user_history_batch.append(user_history_sequence)
    ...
    def _convert_data(
        ...
        if batch_num_ngs:
            ...
            for i in range(instance_cnt):
                this_lengthU = min(history_lengthsU[i], max_seq_length_batch)
                ...
                for index in range(batch_num_ngs + 1):
                    ...
                    user_history_batch_all[
                        i * (batch_num_ngs + 1) + index, :this_lengthU
                    ] = np.asarray(user_history_batch[i][-this_lengthU:], dtype=np.int32)
                    ...
                    mask_u[i * (batch_num_ngs + 1) + index, :this_lengthU] = 1.0
            ...
            res["user_history"] = user_history_batch_all
            # print(user_history_batch_all)
            ...
            mask_u = np.zeros((instance_cnt, max_seq_length_batch)).astype("float32")

            for i in range(instance_cnt):
                this_lengthU = min(history_lengthsU[i], max_seq_length_batch)

                this_length = min(history_lengths[i], max_seq_length_batch)
                user_history_batch_all[i, :this_lengthU] = user_history_batch[i][
                    -this_lengthU:
                ]
                ...
                # import pdb
                # pdb.set_trace()
                # print(history_lengths[i])
                # print(item_cate_history_batch[i][-this_length:])
                # print(item_cate_history_batch_all[i, :this_length]) # 1 all 

                item_cate_history_batch_all[i, :this_length] 

                mask[i, :this_length] = 1.0
                mask_u[i, :this_lengthU] = 1.0
